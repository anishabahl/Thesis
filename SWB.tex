...
To enable physiological parameter extraction from any of these imaging methods, accurate spectra must be extracted from hyperspectral images. annref{19}\textcolor{red}{This requires white balancing to account for lighting conditions, vignetting, and optical transmission through the set-up using a white and dark reference.}
...
%
These relative data can be used to extract ratiometric clinical parameters such as oxygen saturation~\citep{Giannoni2018,MacKenzie2018} \annref{20}\textcolor{red}{, where ratiometric parameters are not dependent on the absolute reflectance intensity due to their calculation being dependent on ratios of measurements.}
However, this may not allow quantitative measurement of all parameters such as chromophore concentrations, including haemoglobin which is therefore often provided on a \annref{21}\textcolor{red}{scale relative to an initial measurement as trend data}~\citep{Kulcke2018}.
...
For demosaicing purposes, in this work, we rely on bilinear interpolation to maintain the spatial resolution of the raw mosaic image but more advanced approaches such as the learning-based one of \citet{Li2021} \annref{23}\textcolor{red}{or more simplistic linear interpolation} could also be employed with the work presented here.
...
%
%
Additionally, noise \selfref{5}\textcolor{red}{$X(i,j,n)$} should be accounted for in both the spatial and spectral domains:
% 
\begin{linenomath*}
\begin{equation}
	W(i,j,n) = MS(n)V(i,j) + \selfref{5}\textcolor{red}{X(i,j,n)}
\label{eq:modelling white references}
\end{equation}
\end{linenomath*}

The assumption of separability is \selfref{6}\textcolor{red}{commonly used to allow the discussion of vignetting and color constancy to be} addressed separately in literature~\citep{Cho2014,Ayala2020,Jiang2019,Yu2004,Foster2011}.
%
%
%
%
...
for each spatio-spectral pixel in the $x, y$ plane there is a temporal distribution of measured intensities, which alternates between background and sterile reference intensities in the $z$ axis. \selfref{4}\textcolor{red}{An example of the sRGB reconstruction of some frames showing the movement of a ruler across the field of view of the camera in a short video can be seen in Figure~\ref{fig:videoframes}.}
\begin{figure}[h]
    \centering \selfref{4}
    \includegraphics[width=\textwidth]{Video_frames.pdf}
    \caption{sRGB reconstructions of three frames taken from a short video of a sterile ruler being passed across the field of view to demonstrate the motion of the ruler.}
...
%
In the generation of synthetic references, this correction is performed on a composite reference which may also present important imperfections due to noise and video input processing\annref{26}\textcolor{red}{, including flaws in pixels not reached by the ruler in the video or due to specular reflection}. We aim to mitigate these through our separable model fitting.
%
%
...
Making use of our convention that $\frac{1}{N} \sum_{n}S(n) = 1$, and assuming a zero mean noise pattern \selfref{5}\textcolor{red}{$X(i,j,n)$}, we find that, up to residual noise, Equation~\eqref{eq:modelling white references} leads to:
%
%
\begin{linenomath*}
\begin{equation}
...
%
%
\selfref{7}\textcolor{red}{In both cases, the least squares fitting routine chosen is SciPy v1.8.1 \newline
\href{https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.least_squares.html}{\texttt{scipy.optimize.least\_squares}} function using it's associated default settings.}
...
The change in lighting conditions has a severe effect on both quantitative and relative data as the shape of the spectra are altered. This effect is significant enough to be easily visually apparent in the sRGB reconstruction. \annref{25}\textcolor{red}{Unstable light sources may present challenges here, however in our work we note that the light sources used show very stable behaviour allowing a single light source characterisation to be used across multiple measurements.}
...
\annref{29}\textcolor{red}{The following chapters show that using quantitative data can allow for lower errors in $StO_2$ estimation. By providing a method for quantitative data extraction which can be easily integrated into a surgical environment, more accurate $StO_2$ could be extracted from clinical data, however this should be further investigated.}
